...
" Delete undo files older than 90 days old
let s:undos = split(globpath(&undodir, '*'), "\n")
call filter(s:undos, 'getftime(v:val) < localtime() - (60 * 60 * 24 * 90)')
call map(s:undos, 'delete(v:val)')

" Auto-fill the Registers {{{
...
nnoremap <leader>tf :set foldmethod=indent<CR>:set foldnestmax=2<CR>
...
" This hack gets rid of the commenting indentation bug where python comments
" would lose all indentation. It's still not perfect, but it's a start!
inoremap # #:<BS>
" }}}
...
" Golang Customiazations {{{
function! CustomizeGolang()
    " No need for a max line length in golang
    " Set a guideline at 100 chars
    set colorcolumn=100
    setl autoread  " So we can run gofmt as we go without rereading
    ...
    " Helpful leader-key shorcuts
    nnoremap <leader>rt :0r ~/.vim/templates/template.go<CR>
    nnoremap <leader>rl :.-1r ~/.vim/templates/license.go<CR>

    " Run gofmt on the code in place
    nnoremap <leader>fm :w<CR>:!gofmt -w %<CR><CR>

    " Autocompletion
    iabbr <silent> p/ fmt.Println()<Left><C-R>=Eatchar('\s')<CR>
    iabbr <silent> pf/ fmt.Printf()<Left><C-R>=Eatchar('\s')<CR>

    " TODO(12/30): Go code autocompletion and auto-setup
...
au BufNewFile,BufRead *.go call CustomizeGolang()
...
au BufWritePre *.sh,*.c,*.h,*.go %s/\s\+$//e
" }}}
